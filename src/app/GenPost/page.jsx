...
//     try {
//       // First search Google for images based on the prompt
//       const searchResponse = await fetch(`/api/searchImage?query=${encodeURIComponent(prompt)}`);
      
//       if (!searchResponse.ok) {
//         throw new Error(`Image search failed with status: ${searchResponse.status}`);
//       }
      
//       const searchData = await searchResponse.json();
      
//       if (!searchData.success || !searchData.image) {
//         throw new Error('No suitable image found for this prompt');
//       }
      
//       // Use the image URL from the search result
//       setGeneratedImage(searchData.image.url);
      
//       // Store in localStorage
//       if (typeof window !== "undefined") {
//         localStorage.setItem("lastGeneratedImage", searchData.image.url);
//         localStorage.setItem("lastImagePrompt", prompt);
//       }
...
//         `Error: ${
//           error.message ||
//           "Failed to find a suitable image for your prompt."
//         }`
//       );
//       setIsLoading(false);
//     }
...
const Page = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      // Load the image - use a consistent key name
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Get the image URL from the search result
      const imageUrl = searchData.image.url;
  ...
  const handleEditImage = () => {
    // Store the image in localStorage for the edit page to access
    // Use the same image that's currently displayed
    if (typeof window !== "undefined" && generatedImage) {
      localStorage.setItem("editImage", generatedImage);
      // Redirect to the image edit page
      router.push("/ImageEdit");
          ...
          <div className="p-4">
            ...
            {isLoading ? (
              ...
              <div className="p-4 overflow-auto max-h-[500px] flex flex-col items-center justify-center">
                {generatedImage && (
                  <div className="flex flex-col items-center">
                    {/* Using div with background image for display */}
                    <div 
                      style={{
                        backgroundImage: `url(${generatedImage})`,
                        backgroundSize: 'contain',
